...
大部分元数据都保存在information_schema中，目前需要掌握的表：
...
在线上进行ALTER TABLE可能会对业务有所影响，可能会阻塞业务的读和查询，从5.6开始支持online ddl可以在做ALTER TABLE的时候业务依然是可读可写的

文档中主要看Permits Concurrent DML项，就是是否可读可写(导致阻塞)，Yes表示没有影响，No的话就可能变成只读

在线ALTER TABLE的时候最好打开这张表对一下明确之后再进行操作
...
分区表是一个很好用的东西，5.7开始还解决了分区表的很多问题
...
这样OrderId做不到全局唯一，只能保证在一个分区内唯一，这是它的一个限制
...
* 自增长，入identity，auto_increment
* 唯一编号
* Max+1
* GUID（Global unique identifier），比如游戏和服的时候用这个很方便
* 应用程序主键生成器
* 推荐每张表都要有一个主键
...
简单点的话可以用下面Data_length / Rows取平均值，但是这种计算方法不太精确
...
精确计算的方法，可以先把一行中每列类型所占用的字节数计算出来：（注意length类型计算的是字符串，不能用于int或者double数字类型，如果能运行那是因为mysql做了隐式的转换，但最好还是不要这么用）
