...
不推荐使用 shell> > slow.log 操作，此操作并不会释放该文件占用的磁盘空间，因为mysql对应该文件的句柄依然是打开的并没有去进行释放

正确的做法是先备份一下 slow.log 文件，然后在MySQL中运行 flush slow logs 关闭 slow.log 日志文件的句柄，这样就会重新打开一个新的slow.log文件的句柄
...
意思是你的 row 至少要被检测(扫描)了 100 行才会记录到慢查询日志
...
比较坑人的参数，如果使用默认值的话使用的是 UTC 时间会慢8小时
...
当前的 mysql 绑定在哪个地址 IP 地址上面，默认是绑定在 *，即所有 IP 的端口上面
