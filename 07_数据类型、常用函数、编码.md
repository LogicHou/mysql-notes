...
ZEROFILL 字段会自动帮你填充 0，只是一个显示的属性而不是数字类型的转换，int(10)、tinyint(4) 中的 10 和 4 本身是没有任何意义，int 的范围是多少那就是多少，(10) 和 (4) 只是表示**显示的宽度**而不是表示能够插入数据的长度，而这个显示的宽度只有加上 ZEROFILL 属性加上才会显示，(10) 和 (4) 的设置才会显得有意义
